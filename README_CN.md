...
空的 `rich-table` 代码块会显示两个按钮：**插入模板**（展示全部功能的完整演示表）或**插入空白表格**，后者会弹出类似 Word/Sheets 的尺寸选择器——鼠标划过 8×6 的网格实时预览、点击确认行列数，也可以直接输入精确数字（超出网格范围时使用）。
...
**15 · 主题切换** — 🎨 图标切换内置主题（`academic`、`plain`）
