...
**6 · 拖拽调整宽高** — 拖拽列标题右边缘调整列宽 · 拖拽行底边缘调整行高

![调整宽高演示](docs/demo-06-resize.gif)

**7 · 标题与底部备注** — 单击内联编辑，Shift+Enter 换行
...
### 调整列宽与行高
悬停到列标题右边缘，出现调整手柄，左右拖拽即可改变列宽；悬停到数据行底边缘，上下拖拽改变行高。拖拽时显示贯穿整列/整行的蓝色指示线实时追踪边界位置。

- 类型列根据最宽的选项标签自动设定最小列宽，确保标签徽章不被截断。
- 宽度和高度持久化到 YAML 头部（`columns[].width`、`rowHeights`）。
